#ifndef IP_CLASSWORK_20_20251028_FILESYSTEM_FILESYSTEM_H_
#define IP_CLASSWORK_20_20251028_FILESYSTEM_FILESYSTEM_H_
...
/**
 * @class FileSystem
 * @brief Clase que implementa un sistema de archivos simple con directorio y
 * bitmap.
 */
...
class FileSystem {
 public:
   ...
   * @param crearNuevoDisco true para crear un nuevo disco, false para cargar
   * uno existente.
   */
  explicit FileSystem(bool crearNuevoDisco = false);
   ...
   * @return Entrada encontrada (si no existe, debe manejarse en
   * implementación).
   */
   ...
   * @detail A diferencia de 'modificar', este método no solo sobreescribe el
   * archivo, sino que también escribe los datos nuevos en bloques distintos. En
   * esencia, este método crea un archivo nuevo con el mismo nombre que el del
   * anterior.
   ...
   * @todo Hacer que sea posible no especificar 'datosNuevos', de manera que el
   * reemplazo es sólo de bloques, y no también de contenido (que haya la opción
   * para ambas cosas)
   */
 ...
 private:
  ...
  /**
   * @brief Nombre del archivo que representa el disco en el sistema de
   * archivos.
   */
  ...
  INodo* directorio;
  /**
   * @brief Bitmap en memoria. Cada bit representa un bloque (0/false = libre,
   * 1/true = ocupado).
   */
  bool* bitmap;
  /**
...
#endif  // IP_CLASSWORK_20_20251028_FILESYSTEM_FILESYSTEM_H_
